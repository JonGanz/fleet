...
Usage:
  fleet-cache ensure <dir> [--force]
      ...
      If <dir>/node_modules was already linked from this exact lockfile
      hash (tracked via a <dir>/node_modules/.fleet-cache-hash marker), this
      is a no-op that leaves node_modules untouched -- including anything
      added since, like an ` + "`npm link`" + ` package symlink. Pass --force to
      rebuild anyway.
...
func main() {
	...
	case "ensure":
		...
		force := fs.Bool("force", false, "rebuild node_modules from the cache even if it's already linked to the current lockfile hash")
		...
		if err := runEnsure(fs.Arg(0), *force); err != nil {
